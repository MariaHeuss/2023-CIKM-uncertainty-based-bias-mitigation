...
if __name__ == "__main__":
    ...
    # load head checkpoint to keep it separate from BERT. This is not necessary if you implement
    # the approximation yourself, but it is necessary if you use the Backpack library.
    if args.saved_head:
    ...
    q_doc = [
        "why is the sky blue?" + tokenizer.sep_token + "due to the rayleigh scattering",
        "why is the sky blue?"
        + tokenizer.sep_token
        + "soccer is a common outdoor activity played across the world",
        "what is a corporation?"
        + tokenizer.sep_token
        + "a corporation is a company or group of people authorized to act as a single entity and recognized as such in law.",
        "what is a corporation?"
        + tokenizer.sep_token
        + "if stock is issued, the corporation will usually be governed by its shareholder",
    ]
    target = torch.tensor([0, 1, 0, 1])
    q_doc_encoded = tokenizer(
        q_doc, return_tensors="pt", max_length=512, padding=True, truncation=True
    )
